...
export function History({ records, open, remove, start, error }: { records: Memory[]; open: (record: Memory) => void; remove: (record: Memory) => void; start: () => void; error: string }) {
  ...
  return <main className="memories-page">
    <div className="memory-heading"><div><div className="eyebrow">每一次喜欢，都有迹可循</div><h2>回望<em>，看见的自己。</em></h2><p>留住那些「原来如此」的瞬间。</p></div><button className="memory-new" onClick={start}><Plus size={17}/>新的看见</button></div>
